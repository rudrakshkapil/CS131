...
def sliding_window(image, base_score, stepSize, windowSize, pixel_per_cell=8):
    ...
    for i in range(0, H+1, stepSize):
        for j in range(0, W+1, stepSize):
            ...
            hog = feature.hog(window, pixels_per_cell=(pixel_per_cell, pixel_per_cell))
            
            score = np.dot(np.transpose(hog), base_score)
...
def pyramid_score(image, base_score, shape, stepSize=20,
    ...
    ### YOUR CODE HERE
    for s,im in images:
        score, r, c, m = sliding_window(im, base_score, stepSize, shape, pixel_per_cell)
...
def shift_heatmap(heatmap, mu):
    ...
    print(np.max(heatmap, axis=None))
    mu = mu/np.max(heatmap, axis=None)
    new_heatmap = heatmap/np.max(heatmap, axis=None)
    new_heatmap = interpolation.shift(new_heatmap, mu, order=1, prefilter=False)
    print(new_heatmap)
    print(mu)
    ### END YOUR CODE
    return new_heatmap
