...
def harris_corners(img, window_size=3, k=0.04):
    ...
    for i in range(H):
        for j in range(W):
            ...
            det = np.linalg.det(M)         
            trace_sq = np.trace(M)**2
            response[i,j] = det - k*trace_sq
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE
    for i in range(N):
        dist = np.copy(dists[i, :])
        j = np.argmin(dist)
        dist = np.sort(dist)
        
        if dist[0] / dist[1] <= threshold:
            matches.append([i, j])
...
def fit_affine_matrix(p1, p2):
    ...
    ### YOUR CODE HERE
    H,res,rank,s = np.linalg.lstsq(p2,p1,rcond=None)
    ### END YOUR CODE
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for it in range(n_iters):
        ...
        # find inliers & check above threshold
    ...
    # recompute least squares on max inliers set
...
def linear_blend(img1_warped, img2_warped):
    ...
    ### YOUR CODE HERE
    mat1 = np.ones((img1_warped.shape))
    dec1 = np.linspace(0,1,int(np.round(np.abs(left_margin-right_margin))))
    mat1[:,left_margin:right_margin] -= dec1
    mat1[:,right_margin:] = 0
            
    mat2 = 1-mat1
    
    print(mat1.shape, img1_warped.shape)
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    output_shape, offset = get_output_space(imgs[ref_ind], [imgs[0],imgs[2],imgs[3]], [np.linalg.inv(H[0]), H[1], H[1].dot(H[2])])
