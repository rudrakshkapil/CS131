...
def compute_distances(X1, X2):
    ...
    #implementation of squareform(dists)
...
def predict_labels(dists, y_train, k=1):
    ...
    idx = np.argsort(dists, axis=1) # order each row - for each test image
    k_indices = idx[:,:k]  # get first k columns of each row - indices to use
   
    closest_y = y_train[k_indices]  # get the actual y_train values
    y_pred = mode(closest_y, axis=1)[0].flatten() # 0->value, 1->count
