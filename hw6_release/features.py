...
class PCA(object):
    ...
    def transform(self, X, n_components):
        ...
        X = X - self.mean
        X_proj = np.dot(X, self.W_pca[:, :n_components]) #NxD . Dxn = Nxn
        # END YOUR CODE
    ...
    def reconstruct(self, X_proj):
        ...
        # Nxn * nxD = NxD
        X = np.dot(X_proj, np.transpose(self.W_pca[:,:n_components]))
        X += self.mean
        # END YOUR CODE
...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            X_i = X[y==i]
            cov_X_i = np.cov(X_i.T)
            scatter_within += cov_X_i
