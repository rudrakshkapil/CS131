...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        ### YOUR CODE HERE
        # get ATA
        # A = [[Ix of those in neighbourhood], [Iy of those]].T
        A = Ix[y-w:y+w+1,x-w:x+w+1].flatten()
        A = np.vstack((A, Iy[y-w:y+w+1,x-w:x+w+1].flatten())).T
...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        A1 = np.sum(Ix2[y1-w: y1+w+1, x1-w: x1+w+1])
        A2 = np.sum(Iy2[y1-w: y1+w+1, x1-w: x1+w+1])
        A3 = np.sum(IxIy[y1-w: y1+w+1, x1-w: x1+w+1])
        G1 = np.hstack((A1,A3))
        G2 = np.hstack((A3,A2))
        G = np.vstack((G1, G2))
        ...
        for k in range(num_iters):
            ...
            bk = np.array([np.sum(Ik*Ix[y1-w: y1+w+1, x1-w: x1+w+1]), np.sum(Ik*Iy[y1-w: y1+w+1, x1-w: x1+w+1])])
            vk = G_inv.dot(bk) 
            ### END YOUR CODE
...
def IoU(bbox1, bbox2):
    ...
    intersection = (y_bottom - y_top) * (x_right - x_left)
